...
export const exercises = sqliteTable("exercises", {
  ...
  createdAt: int("create_at", { mode: "timestamp" }).$defaultFn(
    () => new Date(),
  ),
});
...
export const workoutSessions = sqliteTable("workout_sessions", {
  ...
  userId: int("user_id")
    .notNull()
    .references(() => user.id),
  startedAt: int("started_at", { mode: "timestamp" }).$defaultFn(
    () => new Date(),
  ),
...
export const personalRecords = sqliteTable(
  "personal_records",
  {
    ...
    reps: int("reps").notNull(),
    weightUnit: text("weight_unit").notNull().default("lbs"),
    achievedAt: int("achieved_at", { mode: "timestamp" }).$defaultFn(
      () => new Date(),
    ),
  },
  (table) => [
    uniqueIndex("user_exercise_pr_idx").on(table.userId, table.exerciseId),
  ],
);
